...
:- begin_tests(array_minimal_test).

test(array_write_read, [setup(z3_new_handle(H)), cleanup(z3_free_handle(H))]) :-
    z3_assert(H, a:(array(int,int)) = store(default:(array(int,int)), i:int, 99)),
    z3_assert(H, x:int = select(a, i)),
    z3_check(H, R),
    assertion(R == l_true),
